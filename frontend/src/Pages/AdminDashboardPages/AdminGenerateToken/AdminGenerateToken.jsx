...
const AdminGenerateToken = () => {
  ...
  const handleGenerateToken = async () => {
    setLoading(true);
    try {
      const res = await axios.post("https://api.amdeco-renovation.fr/token/generate-token");
  ...
  return (
    <div className="max-w-md mx-auto mt-10 bg-white p-6 rounded-2xl shadow-lg border border-orange-200">
      <h1 className="text-2xl font-bold mb-4 text-center text-orange-600">Admin Token Generator</h1>
      ...
      {token && (
        <div className="mt-6 space-y-2">
          <div className="flex items-center justify-between bg-gray-100 p-3 rounded-lg">
            ...
            <button onClick={handleCopy} className="btn btn-sm btn-outline btn-info ml-4">
              <FaCopy size={16} />
            </button>
          </div>
          <p className="text-sm text-gray-500">
            <span className="font-semibold">Expires At:</span> {expiresAt}
          </p>
        </div>
      )}
    </div>
