...
const AdminOnSiteVisits = () => {
  ...
  const fetchVisits = async () => {
    try {
      const res = await axios.get("https://construction-project-l8m8.onrender.com/api/onsitevisits");
      setVisits(res.data);
    } catch {
      Swal.fire({
        icon: "error",
  ...
  const handleStatusUpdate = async (id, currentStatus) => {
    ...
    if (confirm.isConfirmed) {
      try {
        await axios.patch(`https://construction-project-l8m8.onrender.com/api/onsitevisits/${id}/status`, {
          status: "solved",
        });
